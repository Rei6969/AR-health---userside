document.addEventListener("DOMContentLoaded", () => {
  ...
  const inpSex = document.querySelector(".inp-sex");
  const forFemaleContainer = document.getElementById("for-female-container");

  inpSex.addEventListener("change", () => {
    if (inpSex.value === "Female") {
      forFemaleContainer.style.display = "block";
    } else {
      forFemaleContainer.style.display = "none";
    }
  });
    ...
    inputCheckboxSecond.forEach((inputContent) => {
      ...
      if (checkboxFirst != null) {
        checkboxFirst.addEventListener("change", () => {
          ...
          if (
            checkboxFirst.id === "checkbox-dysmenorrhea-yes" &&
        ...
        checkboxSecond.addEventListener("change", () => {
          ...
          if (checkboxSecond.id === "checkbox-dysmenorrhea-no") {
            disabled.style.pointerEvents = "none";
            disabled.style.opacity = 0.5;
          }
        });
  ...
  document
    ...
    .addEventListener("click", () => {
      if (validateForm(firstForm)) HandleSecondForm();
  ...
  document
    ...
    .addEventListener("click", () => {
      if (validateForm(secondForm)) HandleThirdForm();
      // else alert("Please fill in all required fields before proceeding.");
    });
  ...
  document
    ...
    .addEventListener("click", () => {
      if (validateForm(thirdForm)) HandleFourthForm();
      // else alert("Please fill in all required fields before proceeding.");
    });
