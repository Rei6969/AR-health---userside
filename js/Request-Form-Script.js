      ...
      if (snapshot.exists()) {
        ...
        btnSubmit.addEventListener("click", () => {
          ...
          const userData = {
            ...
            status: "Pending",
            fileUrl: "",
            requestType: "Medical Form",
          };

          set(child(requestsRef, `${userID}/${requestsID}`), userData).then(
            () => {
              alert(
                "Your request has been successfully submitted. You will receive an update via email, or you may check the files section for updates."
              );
            }
          );
        });
